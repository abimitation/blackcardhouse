...
import { OrderSelect } from "@/db/schema";
import { Badge, Box, Divider, Flex, Pagination, Table, Text } from "@mantine/core";
...
export default function OrderTable({ data }: { data: OrderSelect[] }) {
  const t = useTranslations("Dashboard.orders.table");
  const [pagination, setPagination] = useState<PaginationState>({ pageIndex: 0, pageSize: 10 });
  const columns = useMemo<ColumnDef<OrderSelect>[]>(() => [
    { accessorKey: "id", header: t("id") },
    { accessorKey: "paymentStatus", header: t("payment_status") },
    { accessorKey: "service", header: t("service") },
    { accessorKey: "experiences", header: t("experiences"), cell: ({ row }) => { const { experiences } = row.original; return <Flex gap="xs" w={250} wrap="wrap">{experiences.map((experience) => <Badge key={experience} fw={600} size="sm">{experience}</Badge>)}</Flex>; } },
    ...
    { accessorKey: "updatedAt", header: t("updated_at"), cell: (info) => { const value = info.getValue(); if (typeof value === "string") return new Date(value).toLocaleString(); } },
  ], [t]);
