...
export default function OrderTable({ data }: { data: OrderSelect[] }) {
  ...
  const columns = useMemo<ColumnDef<OrderSelect>[]>(() => [
    ...
    { accessorKey: "paymentStatus", header: "Ödeme durumu" },
    { accessorKey: "service", header: "Hizmet" },
    { accessorKey: "experiences", header: "Deneyimler", cell: ({ row }) => { const { experiences } = row.original; return <Flex gap="xs" w={250} wrap="wrap">{experiences.map((experience) => <Badge key={experience} fw={600} size="sm">{experience}</Badge>)}</Flex>; } },
    ...
    { accessorKey: "updatedAt", header: "Son güncelleme", cell: (info) => { const value = info.getValue(); if (typeof value === "string") return new Date(value).toLocaleString(); } },
  ], []);
